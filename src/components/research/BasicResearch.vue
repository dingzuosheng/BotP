<template>
    <div class="main">
        <div class="content">
            ...
            <div v-if="!this.show">  
                <div>
                    Basic Research Budget: {{ Math.floor(this.basicResearchBudget/Math.pow(10,9)*100)/100 }} billion $
                </div>
                <div class="range">
                    <h3>Basic Research Subsidy  {{ this.basicResearchSubsidy * 100}}%</h3>
                    <input type="range" min="0" max="0.08" step="0.01" v-model="percent" @change="changeBasicResearchSusidyPercent"/><!--value is string-->
                    <div>
                        <p class="text">
    ...
    methods:{
        ...
        draw(){
            ...
            for(let i = 0; i < labels.length; i++){
                coalUses.push(Math.ceil(JSON.parse(localStorage.getItem(labels[i])).basicResearchBudget/Math.pow(10,9)*100)/100)
                console.log(labels[i],localStorage.key(i))
            }
            const dataset = {
                label:'Basic Research Budget(Unit: billion dollars)',
