    ...
    methods:{
        ...
        draw(){
            ...
            for(let i = 0; i < labels.length; i++){
                coalUses.push(Math.ceil(JSON.parse(localStorage.getItem(labels[i])).nuclearResearchBudget/Math.pow(10,9)*100)/100)
                console.log(labels[i],localStorage.key(i))
            }
            const dataset = {
                label:'Nuclear Research Budget(Unit: billion dollars)',
