<template>
    <div class="main">
        <div class="content">
            ...
            <div v-if="!this.show">
                <div>
                    Solar Research Budget: {{ Math.floor(this.solarResearchBudget/Math.pow(10,9)*100)/100 }} billion $
                </div>
                <div class="range">
    ...
    methods:{
        ...
        draw(){
            ...
            for(let i = 0; i < labels.length; i++){
                coalUses.push(Math.ceil(JSON.parse(localStorage.getItem(labels[i])).solarResearchBudget/Math.pow(10,9)*100)/100)
                console.log(labels[i],localStorage.key(i))
            }
            const dataset = {
                label:'Solar Research Budget(Unit: billion dollars)',
